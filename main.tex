...
\subsubsection{CA}
They are responsible for user registration and revocation. Just like many multi-authority architectures like \cite{Chase2007Multi}, the CA does not manage the attribute-related credential issuing task. Instead, it only issues each user a blank credential, and leaves the remaining attribute assignment to other entitites (AA). 
