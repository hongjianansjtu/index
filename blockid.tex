...
\begin{abstract}
	...
	In addition, different from other no-CRL revocation mechanism, AcBF brings much smaller impact on valid user to execute the revocation. %, and with quite a small probability.
...
This paper further assumes the communication links of lightweight node: 1) during long-lifetime interaction, the link is instable and insecure; 2) in its initialization phase, it has adequate resouce to update its local ledger to the latest and correct version, including other variables, e.g., bloom filter and accumulator.
...
When $U_i$ needs to authenticate itself to a lightweight verifier $V$, it should sign a certain message or achanllege with its $sk_i$. This paper focuses on the issue that how $U_i$ convinces $V$ that the relevant $pk_i$ is associated with a valid on-chain certificate.
...
%@misc{implement,
%  author = {OpensourceBooks},
%  title = {blockchain},
%  year = {2018},
%  publisher = {GitHub},
%  journal = {GitHub repository},
%  howpublished = {\url{https://github.com/OpensourceBooks/blockchain}},
...
\begin{table*}[t]
	...
	\begin{tabular}{c | c |c|c | c |c c c}
		\hline
		\multirow{2}*{Scheme} & Lightweight & \multirow{2}*{Revokable} & Revocatin & Additional &\multicolumn{3}{c}{Impact on}\\
		& node \textit{supp.} & & mechanism &method& Verifier  & Owner & CA\\
		\hline
		CertChain & \ding{55}& \checkmark & Bloom Filter\\
		CertLedger & \ding{55}& \checkmark & MHT\\
		ScalaCert &\ding{55}& \checkmark &\\
		PROCESS &\ding{55}& \checkmark &\\
		Jia \textit{et al.} \cite{jiaRedactableBlockchainDecentralized2022} &\ding{55}& \checkmark &\\
		SmartDID & \checkmark & \ding{55} & \textit{n/a}\\ 
		Proposed  & \checkmark & \checkmark & AcBF \\
		\hline
	\end{tabular}
\end{table*}
