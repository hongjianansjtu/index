...
Chen \textit{et al}. \cite{chenCertchainPublicEfficient2018a} proposed a dependability-rank based consensus, thus realized certificate forward traceability. 
...
A totally decentralized identity model called SSI (self-sovereign identity) \cite{foundation_sovrin_2018} was designed by the Sovrin Foundation in 2018. In SSI, users' identities are registered in blockchain by themselves, while authorities are responsible for determining the privilege of each ID with off-chain credentials. This idea has been realized in industrial implementations such as hyperledger indy \cite{indy}, and attracted academic researches, like SmartDID \cite{smartdid}. However, the fully decentralized method does not suit practical identity management, since it is difficult to deal with the trust and revocation.
...
Zhuo \textit{et al}. realized efficient and decentralized key management for VANETs, with additional mechanism of public key registration, update and revocation in \cite{maEfficientDecentralizedKey2020} proposed an efficient decentralized key management mechanism for VANETs with blockchain. 
